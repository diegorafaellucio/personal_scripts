...
model_path = "/home/diego/2TB/yolo/Trains/runs/cogtive_betterbeef_9.0_640_sgd/weights/best.pt"
...
# Função para converter coordenadas YOLO padrão para o formato Label Studio
def convert_yolo_to_labelstudio(results, image_width, image_height):
    annotations = []
    ...
    # Processar cada resultado (normalmente só temos um para uma única imagem)
    for result in results:
        # Verificar se o resultado tem detecções
        if not hasattr(result, 'boxes') or result.boxes is None:
            print("Nenhuma detecção encontrada no resultado.")
            continue
            
        # YOLO padrão fornece caixas no formato xyxy (x1, y1, x2, y2)
        if result.boxes.xyxy is not None and len(result.boxes.xyxy) > 0:
            ...
            for i, (box, conf, cls) in enumerate(zip(result.boxes.xyxy, result.boxes.conf, result.boxes.cls)):
                # Extrair valores da caixa delimitadora
                x1, y1, x2, y2 = box.tolist()
                confidence = float(conf)
                class_id = int(cls)
                class_name = result.names[class_id].lower()  # Converter para minúsculo
                
                # Calcular centro, largura e altura
                x_center = (x1 + x2) / 2
                y_center = (y1 + y2) / 2
                width = x2 - x1
                height = y2 - y1
                ...
                print(f"Classe: {class_name}, Box: ({x1}, {y1}, {x2}, {y2}), Centro: ({x_center}, {y_center})")
                
                # Converter coordenadas absolutas para percentuais (0-100%)
                x_percent = (x_center / image_width) * 100
                y_percent = (y_center / image_height) * 100
                width_percent = (width / image_width) * 100
                height_percent = (height / image_height) * 100
                
                # Formato Label Studio para bounding box (sem rotação)
                label_studio_format = {
                    "id": f"result_{i+1}",
                    "type": "rectanglelabels",
                    "value": {
                        "x": max(0, x_percent - width_percent/2),
                        "y": max(0, y_percent - height_percent/2),
                        "width": width_percent,
                        "height": height_percent,
                        "rectanglelabels": [class_name]  # Já está em minúsculo
                    },
                    "score": confidence,
                    "from_name": "label",
                    "to_name": "image",
            ...
            print("Formato de caixas xyxy não encontrado ou vazio.")
...
def send_prediction_to_labelstudio(task_id, predictions, headers):
    ...
    data = {
        "task": task_id,
        "model_version": "yolov8_standard_best",
        "result": predictions
    }
    ...
    if True:
        ...
        for task in tasks_to_process:
            ...
            try:
                ...
                print(f"Executando modelo YOLO em {temp_path}...")
                # Fazer predições com o modelo YOLO (usando detecção padrão)
                results = model(temp_path, verbose=DEBUG_MODE)  # Detecção padrão, não OBB
                ...
                # Converter resultados YOLO para formato Label Studio
                print("Convertendo resultados para formato Label Studio...")
                annotations = convert_yolo_to_labelstudio(results, image_width, image_height)
