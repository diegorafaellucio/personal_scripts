...
# ==== Export Pascal VOC XML and images from a Label Studio project ====
"""
On execution, this script exports annotated tasks from the configured Label Studio
project into two folders:
 - Images -> /home/diego/2TB/datasets/COGTIVE/KOVI/GENERAL/2.0/IMAGES/
 - Pascal VOC XML -> /home/diego/2TB/datasets/COGTIVE/KOVI/GENERAL/2.0/ANNOTATIONS/

After finishing the export, the script exits immediately so that the legacy
...
LS_BASE_URL = "https://internal-label.cogtive.com"
PROJECT_ID = int(os.getenv("LS_PROJECT_ID", "2"))
TOKEN = os.getenv('LABEL_STUDIO_TOKEN', 'a27c0b74353fe5f041d9d54d8323d8dfb3457c64')

OUTPUT_IMAGES_DIR = Path("/home/diego/2TB/datasets/COGTIVE/BETTER_BEEF/10/IMAGES")
OUTPUT_ANN_DIR = Path("/home/diego/2TB/datasets/COGTIVE/BETTER_BEEF/10/ANNOTATIONS")
OUTPUT_IMAGES_DIR.mkdir(parents=True, exist_ok=True)
OUTPUT_ANN_DIR.mkdir(parents=True, exist_ok=True)

EXPORT_MAX_THREADS = int(os.getenv('LS_MAX_THREADS', '8'))

# ---------- Logging setup ----------
LOG_LEVEL = os.getenv('LS_LOG_LEVEL', 'INFO').upper()
LOG_FILE = os.getenv('LS_LOG_FILE', str(OUTPUT_ANN_DIR.parent / 'export_voc.log'))

logger = logging.getLogger('ls_export_voc')
if not logger.handlers:
    ...
    try:
        ...
        # If file handler can't be created, continue with console logs only
        logger.warning(f"Couldn't create log file at {LOG_FILE}, continuing without file logs.")

# Session with retries for export
export_retries = Retry(total=5, backoff_factor=0.5, status_forcelist=[429, 500, 502, 503, 504], allowed_methods=["GET"])
export_adapter = HTTPAdapter(max_retries=export_retries, pool_connections=100, pool_maxsize=100)
export_session = requests.Session()
export_session.mount("https://", export_adapter)
export_session.mount("http://", export_adapter)
EXPORT_HEADERS = {"Authorization": f"Token {TOKEN}"}

_export_thread_local = threading.local()

def get_export_thread_session():
    s = getattr(_export_thread_local, 'session', None)
    if s is None:
        s = requests.Session()
        s.mount("https://", export_adapter)
        s.mount("http://", export_adapter)
        _export_thread_local.session = s
    return s
...
def paged_tasks():
    ...
    page_size = 200
    while True:
        params = {"page": page, "page_size": page_size, "annotations__isnull": "false"}
        url = f"{LS_BASE_URL}/api/projects/{PROJECT_ID}/tasks/"
        logger.info(f"Requesting tasks page={page} size={page_size}")
        resp = export_session.get(url, headers=EXPORT_HEADERS, params=params, timeout=60)
        ...
        if not tasks:
            logger.info("No more tasks returned by API.")
            break
        for t in tasks:
            yield t
        if not has_next:
...
def ensure_unique_filename(base_name: str, suffix: str, existing: set) -> str:
    name = base_name
    if name in existing:
...
def to_voc_xml(filename: str, img_path: Path, width: int, height: int, depth: int, rects: list) -> ET.ElementTree:
    ann = ET.Element("annotation")
    ET.SubElement(ann, "folder").text = OUTPUT_IMAGES_DIR.name
    ET.SubElement(ann, "filename").text = filename
    ET.SubElement(ann, "path").text = str(img_path)
    source = ET.SubElement(ann, "source")
    ET.SubElement(source, "database").text = "LabelStudio"
    ...
    for r in rects:
        ...
        ET.SubElement(bb, "xmin").text = str(r["xmin"])
        ET.SubElement(bb, "ymin").text = str(r["ymin"])
        ET.SubElement(bb, "xmax").text = str(r["xmax"])
        ET.SubElement(bb, "ymax").text = str(r["ymax"])
    return ET.ElementTree(ann)

def process_task(task, existing_files: set):
    image_url = (task.get("data") or {}).get("image")
    if not image_url:
        return False
    if not image_url.startswith("http"):
        image_url = f"{LS_BASE_URL}{image_url}"
    parsed = urlparse(image_url)
    base = os.path.basename(parsed.path) or f"task_{task.get('id', 'unknown')}.jpg"
    stem, ext = os.path.splitext(base)
    if not ext:
        ext = ".jpg"
    unique_stem = ensure_unique_filename(stem, str(task.get("id")), existing_files)
    filename = f"{unique_stem}{ext}"
    img_path = OUTPUT_IMAGES_DIR / filename
    # Download image
    sess = get_export_thread_session()
    logger.debug(f"Downloading image for task {task.get('id')} -> {filename}")
    r = sess.get(image_url, headers=EXPORT_HEADERS, timeout=60)
    r.raise_for_status()
    img_bytes = r.content
    ...
    if img is None:
        logger.warning(f"Failed to decode image for task {task.get('id')} ({image_url})")
        return False
    h, w = img.shape[:2]
    depth = 1 if len(img.shape) == 2 else img.shape[2]
    # Pick annotation
    ...
    for item in result:
        ...
        xmin = max(1, int(round((x / 100.0) * w)))
        ymin = max(1, int(round((y / 100.0) * h)))
        xmax = min(w, int(round(((x + width) / 100.0) * w)))
        ymax = min(h, int(round(((y + height) / 100.0) * h)))
        # Sanity check
        if xmax <= xmin or ymax <= ymin:
            continue
        rects.append({"label": label, "xmin": xmin, "ymin": ymin, "xmax": xmax, "ymax": ymax})
    # Write XML
    xml_tree = to_voc_xml(filename, img_path, w, h, depth, rects)
    xml_path = OUTPUT_ANN_DIR / f"{unique_stem}.xml"
    xml_tree.write(xml_path, encoding="utf-8", xml_declaration=True)
    logger.debug(f"Wrote XML {xml_path}")
    return True

def _export_main():
    logger.info(f"Starting export from project {PROJECT_ID} -> IMAGES={OUTPUT_IMAGES_DIR} ANN={OUTPUT_ANN_DIR}")
    tasks = list(paged_tasks())
    if not tasks:
        msg = "No annotated tasks found."
        logger.info(msg)
        print(msg)
        return
    existing = set()
    success = 0
    with ThreadPoolExecutor(max_workers=EXPORT_MAX_THREADS) as ex, tqdm(total=len(tasks), desc="Exporting (VOC + images)") as pbar:
        futures = [ex.submit(process_task, t, existing) for t in tasks]
        for fut in as_completed(futures):
            ...
            try:
                ...
                ok = False
                logger.exception("Error while processing a task")
            if ok:
                success += 1
            pbar.update(1)
    logger.info(f"Export done. {success}/{len(tasks)} tasks exported.")
    logger.info(f"Images: {OUTPUT_IMAGES_DIR} | Annotations: {OUTPUT_ANN_DIR}")
    print(f"Export done. {success}/{len(tasks)} tasks exported to:")
