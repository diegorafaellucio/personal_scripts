...
def draw_detections(image_path, detections):
    ...
    for detection in detections:
        ...
        if confidence >= 0.20:
            ...
            to_print = '{} {}'.format(label, round(confidence, 2))
...
def process_data(images_input_path, image, classifier, images_output_path):
    image_path = os.path.join(images_input_path, image)

    img = cv2.imread(image_path)

    detection_results = classifier.detect(img)

    filtered_detections = remove_region_intersection(detection_results)
...
def process_model(train_name, network_name, planta, images_input_path, images_output_path, classifier_model_path, device='cuda:0'):
    classifier = Classifier(classifier_model_path,
                            device=device, max_det=10, image_size=[640, 640])
    ...
    for image in tqdm.tqdm(images):
        futures.append(executor.submit(process_data, images_input_path, image, classifier, images_output_path))
...
if __name__ == '__main__':
    ...
    tamanho_imagem = '416'
    rede = 'V5_MEDIUM'
    path_modelo = '/home/diego/2TB/yolo/new_v5/runs/train/lesao_barra_mansa_v5_extra_large_416/weights/best.pt'

    process_model(treino, rede, 'BARRA_MANSA', '/home/diego/2TB/datasets/eco/AUDITORIA_BOVINOS/BARRA_MANSA/2024/03/06',
                  'BARRA_MANSA_LESAO',
                  path_modelo)
    # 
